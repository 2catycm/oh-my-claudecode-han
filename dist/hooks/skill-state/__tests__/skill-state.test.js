...
describe('skill-state', () => {
    ...
    describe('getSkillProtection', () => {
        ...
        it('strips omc-han: prefix', () => {
            expect(getSkillProtection('omc-han:plan')).toBe('medium');
            expect(getSkillProtection('omc-han:ralph')).toBe('none');
        });
        ...
        it('returns none for project custom skills with same name as OMC skills (issue #1581)', () => {
            // rawSkillName without omc-han: prefix → project custom skill
        ...
        it('returns protection for OMC skills when rawSkillName has prefix', () => {
            expect(getSkillProtection('plan', 'omc-han:plan')).toBe('medium');
            expect(getSkillProtection('deepinit', 'omc-han:deepinit')).toBe('heavy');
        });
    ...
    describe('writeSkillActiveState', () => {
        ...
        it('strips namespace prefix from skill name', () => {
            const state = writeSkillActiveState(tempDir, 'omc-han:plan', 'session-1');
            expect(state.skill_name).toBe('plan');
        });
        ...
        it('writes state for OMC skills when rawSkillName has prefix', () => {
            const state = writeSkillActiveState(tempDir, 'plan', 'session-1', 'omc-han:plan');
            expect(state).not.toBeNull();
            expect(state.skill_name).toBe('plan');
            expect(state.max_reinforcements).toBe(5);
        });
    ...
    describe('upsertWorkflowSkillSlot — pure helper', () => {
        ...
        it('strips omc-han: prefix from skill name', () => {
            const state = upsertWorkflowSkillSlot(emptySkillActiveStateV2(), 'omc-han:ralph', {
                session_id: 's1',
                mode_state_path: 'r.json',
                initialized_mode: 'ralph',
                initialized_state_path: '',
            ...
            expect(state.active_skills['omc-han:ralph']).toBeUndefined();
        });
