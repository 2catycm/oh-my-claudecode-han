...
describe("subagent-tracker", () => {
    ...
    describe("getAgentDashboard", () => {
        ...
        it("should strip omc-han: prefix from agent type", () => {
            const state = {
                agents: [
                    {
                        agent_id: "test-123",
                        agent_type: "omc-han:architect-high",
                        started_at: new Date().toISOString(),
                        parent_mode: "none",
                        status: "running",
                    },
            ...
            expect(dashboard).not.toContain("omc-han:architect-high");
        });
    });
    ...
    describe("processSubagentStop", () => {
        ...
        it("reconciles an unmatched fork stop by agent_type when one type matches", () => {
            for (const [id, type] of [
                ["exec-1", "omc-han:executor"],
                ["explore-1", "omc-han:explorer"],
            ]) {
                processSubagentStart({
