...
export function getAgentDashboard(directory, sessionId) {
    ...
    for (const agent of running) {
        ...
        const shortType = agent.agent_type.replace("omc-han:", "");
...
export function getAgentObservatory(directory, sessionId) {
    ...
    for (const intervention of interventions.slice(0, 3)) {
        const shortType = intervention.agent_type.replace("omc-han:", "");
        ...
        if (agents.length > 1) {
            ...
            for (let i = 1; i < agents.length; i++) {
                interventions.push({
                    ...
                    reason: `File conflict on ${file} with ${agents[0].type.replace("omc-han:", "")}`,
                    suggested_action: "warn",
                    auto_execute: false,
                });
            }
...
export function detectFileConflicts(directory, sessionId) {
    ...
    for (const agent of running) {
        for (const file of agent.file_ownership || []) {
            ...
            fileToAgents
                .get(file)
                .push(agent.agent_type.replace("omc-han:", ""));
        }
    }
    const conflicts = [];
    for (const [file, agents] of fileToAgents) {
