 ...
 * Ralph Hook
 *
 * Self-referential work loop that continues until cancelled via /omc-han:cancel.
...
export function createRalphLoopHook(directory) {
    const startLoop = (sessionId, prompt, options) => {
        ...
        if (isUltraQAActive(directory, sessionId)) {
            console.error("Cannot start Ralph Loop while UltraQA is active. Cancel UltraQA first with /omc-han:cancel.");
            return false;
        }
