...
| Planning | Hypothesis → structured plan | omc-han:planner | opus |
| Architecture Review | 6-point plan review | omc-han:architect | opus |
| Critic Review | Harness rule enforcement | omc-han:critic | opus |
| Execution | Implement plan + run benchmark | omc-han:executor | opus |
| Git Operations | Atomic merge/tag/PR | omc-han:git-master | sonnet |
...
- **Worktree setup** (SKILL.md creates before each executor):
  ...
  ```
- **Winner merges** via `omc-han:git-master`:
  ```
...
Spawn N `omc-han:planner`(model=opus) agents in parallel (N = `number_of_agents` from settings).
...
**6a. Architecture Review**: Spawn `omc-han:architect` with the plan + 6-point checklist:
...
**6b. Critic Review**: Spawn `omc-han:critic` with the plan + harness rules:
...
For each approved plan, spawn `omc-han:executor`(model=opus) in parallel.
...
4. **Ranked-candidate loop** — for each candidate in rank order (best first):
   ...
   b. **Merge** via `omc-han:git-master`: `git merge experiment/round_{n}_executor_{id} --no-ff -m "Iteration {n}: {hypothesis} (score: {before} → {after})"`
...
4. Print summary report:
   ...
   ```
5. Run `/omc-han:cancel` for clean state cleanup
