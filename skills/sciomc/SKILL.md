...
```
/omc-han:sciomc <goal>                    # Standard research with user checkpoints
/omc-han:sciomc AUTO: <goal>              # Fully autonomous until complete
/omc-han:sciomc status                    # Check current research session status
/omc-han:sciomc resume                    # Resume interrupted research session
/omc-han:sciomc list                      # List all research sessions
/omc-han:sciomc report <session-id>       # Generate report for session
```

### Quick Examples

```
/omc-han:sciomc What are the performance characteristics of different sorting algorithms?
/omc-han:sciomc AUTO: Analyze authentication patterns in this codebase
/omc-han:sciomc How does the error handling work across the API layer?
```
...
Task(subagent_type="omc-han:scientist", model="haiku", prompt="[RESEARCH_STAGE:1] Investigate...")

// Stage 2 - Standard analysis
Task(subagent_type="omc-han:scientist", model="sonnet", prompt="[RESEARCH_STAGE:2] Analyze...")

// Stage 3 - Complex reasoning
Task(subagent_type="omc-han:scientist", model="opus", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
Task(subagent_type="omc-han:scientist", model="sonnet", prompt="
...
4. **Cancellation:** `/omc-han:cancel` or "stop", "cancel"

### AUTO Mode Example

```
/omc-han:sciomc AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="omc-han:scientist", model="haiku", prompt="[STAGE:1] Analyze src/api/...")
Task(subagent_type="omc-han:scientist", model="haiku", prompt="[STAGE:2] Analyze src/utils/...")
Task(subagent_type="omc-han:scientist", model="haiku", prompt="[STAGE:3] Analyze src/components/...")
```
...
Task(subagent_type="omc-han:scientist", model="sonnet", prompt="[HYPOTHESIS:A] Test if caching improves...")
Task(subagent_type="omc-han:scientist", model="sonnet", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Task(subagent_type="omc-han:scientist", model="sonnet", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Task(subagent_type="omc-han:scientist", model="opus", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/omc-han:sciomc status` | Show current session progress |
| `/omc-han:sciomc resume` | Resume most recent interrupted session |
| `/omc-han:sciomc resume <session-id>` | Resume specific session |
| `/omc-han:sciomc list` | List all sessions with status |
| `/omc-han:sciomc report <session-id>` | Generate/regenerate report |
| `/omc-han:sciomc cancel` | Cancel current session (preserves state) |
